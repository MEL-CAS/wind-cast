...
def compute_sample_weights(target_values, n_bins=20):
    """Poids inversement proportionnel à la fréquence de la tranche — les valeurs
    rares (vent fort sur un site comme Bordeaux) sont sur-représentées pendant
    l'entraînement, sans dupliquer aucune donnée."""
    counts, edges = np.histogram(target_values, bins=n_bins)
    bin_idx = np.clip(np.digitize(target_values, edges[:-1]) - 1, 0, n_bins - 1)
    weights = 1.0 / (counts[bin_idx] + 1)
    return weights


def run_optuna(zones, features, target_col, scX, scY, n_trials=15, trial_callback=None,
                search_epochs=3, max_search_rows=6000):
    """TPE, version vectorisée (fenêtres construites une fois via numpy, mises en
    cache par valeur de window). Volontairement PAS repondéré ici — la recherche
    d'hyperparamètres compare des configurations entre elles, la repondération
    n'intervient que dans l'entraînement final (train_full_pipeline)."""
...
def train_full_pipeline(zones, features, target_col, best_params, scX, scY, epoch_callback=None,
                       reweight=True):
    """Entraînement final GRU (early stopping, échantillonnage repondéré vers les
    valeurs rares si reweight=True) + XGBoost sur résidus (décalage causal
    X_raw[W-1:-1], repondéré lui aussi via sample_weight)."""
    ...
    if reweight:
        # Poids calculés sur la cible DÉNORMALISÉE (zones["gru_train"][target_col]),
        # pas sur y_tr scalé — les tranches doivent rester en unités réelles (m/s)
        sample_weights = compute_sample_weights(zones["gru_train"][target_col].values[W:])
    ...
    if reweight:
        # Poids doux (clip 0.5-5) plutôt que 1/fréquence brute — XGBoost est déjà
        # plus sensible aux poids extrêmes que le sampler du GRU
